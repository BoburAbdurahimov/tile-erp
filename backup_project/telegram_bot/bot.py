...
WEBAPP_HTTPS_URL = "https://league-sources-shareholders-stanford.trycloudflare.com/webapp"
...
def set_user_lang(tg_id: int, username: str, first_name: str, lang: str):
    db = SessionLocal()
    try:
        user = db.query(TelegramUser).filter(TelegramUser.telegram_id == tg_id).first()
        if not user:
            user = TelegramUser(telegram_id=tg_id, username=username, first_name=first_name, language=lang)
            db.add(user)
        else:
            user.language = lang
        db.commit()
...
def get_main_keyboard(lang: str) -> ReplyKeyboardMarkup:
    t = BOT_TEXTS[lang]
    keyboard = [
        [KeyboardButton(text=t["btn_webapp"], web_app=WebAppInfo(url=WEBAPP_HTTPS_URL))],
        [KeyboardButton(text=t["menu_warehouse"]), KeyboardButton(text=t["menu_cash"])],
        [KeyboardButton(text=t["menu_production"]), KeyboardButton(text=t["menu_balances"])],
        [KeyboardButton(text=t["menu_finance"]), KeyboardButton(text=t["btn_change_lang"])]
    ]
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Assalomu alaykum, {user.first_name}!\nЗдравствуйте, {user.first_name}!\n\nIltimos, muloqot tilini tanlang / Выберите язык:",
...
async def lang_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    set_user_lang(user.id, user.username, user.first_name, lang)
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = SessionLocal()
    user_role = "Ish boshqaruvchi"
    try:
        db_user = db.query(TelegramUser).filter(TelegramUser.telegram_id == user.id).first()
        if not db_user:
            count = db.query(func.count(TelegramUser.id)).scalar() or 0
            user_role = "Admin" if count == 0 else "Ish boshqaruvchi"
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "ombor" in text or "склад" in text:
        ...
        await update.message.reply_text(BOT_TEXTS[lang]["welcome"], reply_markup=get_main_keyboard(lang), parse_mode="Markdown")
