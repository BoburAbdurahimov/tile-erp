const KassaModule = {
    ...
    showModal(
      ...
      `
        <form id="kassa-tx-form">
          ...
          <div class="form-group">
            <label class="form-label">Bog'langan Kontragent (Ixtiyoriy)</label>
            <select id="tx-counterparty" class="form-control">
              <option value="">Tanlanmagan (Boshqa manba)</option>
            </select>
          </div>
          <div class="form-group">
            <label class="form-label">${t('th_description')} (Ixtiyoriy)</label>
            <textarea id="tx-desc" class="form-control" rows="2" placeholder="To'lov maqsadi yoki izoh yozing..."></textarea>
          </div>
        </form>
      `,
      async () => {
        ...
        const cpId = parseInt(document.getElementById("tx-counterparty").value) || null;
        ...
        try {
          await API.createCashTransaction({
            register_id: regId,
            type,
            source_type: cpId ? "counterparty" : "other",
  ...
  async populateCounterpartiesForTx() {
    const cpSelect = document.getElementById("tx-counterparty");
    if (!cpSelect) return;
    try {
      const cps = await API.getCounterparties();
      cpSelect.innerHTML += cps.map(c => `
        <option value="${c.id}">${c.code} - ${c.name} (${c.type === 'client' ? 'Mijoz' : 'Postavshik'})</option>
      `).join("");
    } catch (e) {
      console.error(e);
    }
